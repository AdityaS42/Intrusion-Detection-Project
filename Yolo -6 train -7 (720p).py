...
batch_size = 2
epochs = 3
input_shape = (64, 720, 1280, 3)  # Adjust the input shape for color (3 channels) 720p data (16:9 aspect ratio)
num_classes = len(os.listdir(train_dir))  # Number of action classes

# Data preprocessing function for 720p color frames
def preprocess_video(video_path):
    ...
    while True:
        ...
        if not ret:
            break
        frame = cv2.resize(frame, (input_shape[2], input_shape[1]))  # Resize to 1280x720
        frames.append(frame)
...
# Define the 3D CNN model for 720p color frames
...
model.save('action_recognition_model_720p_color.h5')
