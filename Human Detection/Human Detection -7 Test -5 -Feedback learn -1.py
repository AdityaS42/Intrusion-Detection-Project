...
i3d_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
...
feedback_period = 100  # Adjust how often to update the model based on feedback
feedback_dataset = []  # List to store feedback data for fine-tuning
    ...
    if filename.endswith(".mp4"):
        ...
        while True:
            ...
            if bounding_boxes:
                ...
                # Add feedback data to the dataset for fine-tuning
                feedback_dataset.append((frame_for_recognition, predicted_class))

                # Check if it's time to update the model
                if frame_number % feedback_period == 0:
                    # Convert feedback dataset to NumPy arrays
                    feedback_images, feedback_labels = zip(*feedback_dataset)
                    feedback_images = np.vstack(feedback_images)
                    feedback_labels = np.array(feedback_labels)

                    # Fine-tune the model with feedback data
                    i3d_model.fit(feedback_images, feedback_labels, epochs=1)  # You can adjust the number of epochs
