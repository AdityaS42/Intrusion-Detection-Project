...
# Define action rules based on bounding boxes and motion
action_rules = {
    "Run": lambda bounding_boxes: any(box[2] - box[0] > box[3] - box[1] for box in bounding_boxes),
    "Jump": lambda bounding_boxes: any(box[1] < jump_roi_y for box in bounding_boxes),
    "Climb": lambda bounding_boxes: any(box[0] < climb_roi_x for box in bounding_boxes),
    # Add more rules for other actions as needed
}

# Define ROI for specific actions (e.g., climbing)
climb_roi_x = 640  # Adjust this value to define the region for climbing detection
    ...
    if filename.endswith(".mp4"):
        ...
        while True:
            ...
            if bounding_boxes:
                ...
                # Check for actions based on defined rules
                for action, rule in action_rules.items():
                    if rule(bounding_boxes):
