...
# Load the pre-trained InceptionV3 model without top classification layer
...
# Create the modified InceptionV3 model
...
# Dynamic learning flag
dynamic_learning = True  # Start with dynamic learning enabled
            ...
            if bounding_boxes:
                ...
                if dynamic_learning:
                    ...
                    if last_frame:
                        ...
                        feedback = input("Enter the action label for this frame (or 'done' to finish dynamic learning): ")
                        if feedback.lower() == "done":
                            dynamic_learning = False
                        elif feedback in action_labels:
                            current_action = feedback
                            # Learn from the feedback only if it matches the predicted action
                            if last_frame and current_action == action_labels[np.argmax(predictions)]:
                                predictions = np.zeros_like(predictions)
                                predictions[action_labels.index(current_action)] = 1
                                i3d_model.fit(frame_for_recognition, predictions, epochs=1, verbose=0)
